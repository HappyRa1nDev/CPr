...
STDMETHODIMP CoCar::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags,
    ...
    UINT* puArgErr)
{
    if (dispIdMember == 1) // IEngine::SpeedUp
    {
        return SpeedUp();
    }
    else if (dispIdMember == 2) // IEngine::GetMaxSpeed
    {
    ...
    else if (dispIdMember == 3) // IEngine::GetCurSpeed
    {
        if (pVarResult != NULL && pDispParams != NULL && pDispParams->cArgs == 1 &&
            pDispParams->rgvarg != NULL && pDispParams->rgvarg->vt == VT_BYREF)
        {
    ...
    else if (dispIdMember == 4) // IStats::DisplayStats
    {
        return DisplayStats();
    }
    else if (dispIdMember == 5) // IStats::GetPetName
    {
    ...
    else if (dispIdMember == 6) // ICreateCar::SetPetName
    {
    ...
    else if (dispIdMember == 7) // ICreateCar::SetMaxSpeed
    {
        if (pDispParams != NULL && pDispParams->cArgs == 1 && pDispParams->rgvarg != NULL &&
